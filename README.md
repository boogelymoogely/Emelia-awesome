...
For me, that would be /home/emelia/.config/awesome. It'll be different if you have a different username, which is highly likely, just change it accordingly. If you don'ot have the "awesome" directory created, no problem, just make one then copy the files into it and edit as needed. If you experience issues with tearing, use the version with "tearingfix" in the name. That should fix Compton/Picom and make it use Vsync, not sure how it'll be on other GPUs, but this fixes tearing issues for me on NVIDIA. 
